...
@group(0) @binding(0) var<storage, read_write> exp_data : array<atomic<u32>>; //Output data
...
@group(0) @binding(1) var<storage, read_write> metadata : array<u32>; //input data
@group(0) @binding(2) var<storage, read_write> hash_data : array<u32>; //input data
@group(0) @binding(3) var<storage, read_write> target_data : array<u32>; //input data
...
)
{
    ...
    for (var i = 0u; i < total_hashes_todo; i++)
    {
        ...
        if validate(y)
        {
            atomicStore(&exp_data[0], y); //Use a pointer to data[word_index]
            atomicStore(&exp_data[1], 1u); //Use a pointer to data[word_index]
        }

        // context[y] = bite_index;
    }
}
